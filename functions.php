...
if ( ! function_exists( 'arsha_setup' ) ) :
	...
	function arsha_setup() {
		...
		register_nav_menus( array(
			...
			'menu-2' => esc_html__( 'Footer', 'arsha' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'arsha_custom_background_args', array(
...
function arsha_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Extra Sidebar', 'arsha' ),
		'id'            => 'sidebar-2',
		'description'   => esc_html__( 'Add widgets here.', 'arsha' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
...
function arsha_scripts() {
	...
	wp_enqueue_script( 'arsha-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'arsha-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
